...
export const useSession = defineStore('session', {
    ...
    actions: {
        ...
        async SignUp(firstName: string, lastName: string, handle: string, password: string, email: string, pic: string, id: number ) {

            const messages = useMessages();
        
            try {
                
                const user = await this.api("users/", { firstName, lastName, handle, password, email, pic, id });
        
                if(user) {
                   console.log('Register',user);
                    messages.notifications.push({
                        type: "success",
                        message: "Registered successfull you can now login",
                    });
                }
